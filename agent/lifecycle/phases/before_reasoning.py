...
class _SyncToolContextModule:
    ...
    async def run(self, frame: BeforeReasoningFrame) -> BeforeReasoningFrame:
        ...
        session_metadata = (
            ...
            if isinstance(state.session.metadata, dict)
            else {}
        )
        ...
        self._tools.set_context(
            ...
            role_id=role_id,
            group_member_id=str(session_metadata.get("group_member_id") or "").strip(),
            group_id=str(session_metadata.get("group_id") or "").strip(),
            is_group_chat="true" if session_metadata.get("is_group_chat") else "",
