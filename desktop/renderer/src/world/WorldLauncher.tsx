...
/** Galgame-style Story launcher with one compact, viewport-safe menu surface. */
  ...
  return (
    <section className="relative h-full min-h-0 overflow-hidden" data-testid="world-launcher">
      ...
      <div className="absolute left-[clamp(16px,4vw,32px)] top-[clamp(16px,4vh,32px)] z-10">
        ...
        <p className="mt-2 text-sm text-[#4B3B3E]">每段剧情，都从一次选择开始。</p>
      </div>
        ...
        {loadOpen ? (
          <section data-testid="world-load-list">
            <div className="mb-3 flex items-center gap-2">
              <button className="grid h-9 w-9 place-items-center rounded-md text-white/65 hover:bg-white/10 hover:text-white" type="button" aria-label="返回剧情主菜单" onClick={() => setLoadOpen(false)}><ArrowLeft /></button>
              <h2 className="m-0 font-serif text-lg text-[#F8F0E8]">选择剧情</h2>
            </div>
            {worlds.length ? (
              ...
              <p className="m-0 py-5 text-center text-sm text-white/55">还没有已保存的剧情。</p>
            )}
          </section>
        ) : (
          <nav className="grid gap-1" aria-label="剧情主菜单">
            <button className={menuButtonClass} type="button" disabled={busy} onClick={onCreateWorld}><Plus className="text-[#DDA27E]" size={22} /><span className="font-serif">创建剧情</span></button>
            <button className={menuButtonClass} type="button" disabled={busy} onClick={() => setLoadOpen(true)}><FolderOpen className="text-[#DDA27E]" size={22} /><span className="font-serif">加载剧情</span></button>
