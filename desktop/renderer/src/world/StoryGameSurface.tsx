...
export function StoryGameSurface({ world, busy, error, characterAvatarUrl, onSubmitAction, onOpenArchive, onOpenSettings, onExit }: StoryGameSurfaceProps) {
  const [action, setAction] = useState("");
  const latestBeat = world.scene.beats[world.scene.beats.length - 1] ?? null;
  const speakerName = latestBeat?.speakerName ?? "";
  const visibleText = latestBeat?.content || world.premise;
  const canSubmit = world.status !== "running" && !busy && Boolean(action.trim());

  async function submit() {
    if (!canSubmit) return;
    if (await onSubmitAction(action.trim())) setAction("");
  }
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 z-20 px-[clamp(20px,8vw,120px)] pb-[clamp(24px,6vh,64px)]">
        <section className="pointer-events-auto mx-auto max-w-5xl border border-white/25 bg-[#10191E]/75 px-[clamp(20px,4vw,40px)] py-6 shadow-[0_16px_46px_rgba(5,11,15,0.32)] backdrop-blur-md">
          ...
          <div className="mt-5 flex items-end gap-3 border-t border-white/15 pt-4">
            <AutosizeTextarea className="min-h-11 w-full bg-transparent px-1 py-2 text-sm leading-6 text-white placeholder:text-white/55 focus:outline-none" containerClassName="min-h-11 flex-1" mirrorClassName="px-1 py-2 text-sm leading-6" value={action} placeholder={world.scene.actionPrompt} onChange={(event) => setAction(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void submit(); } }} />
          ...
          {world.status === "running" ? <p className="m-0 mt-3 text-xs text-[#F1C4A8]">剧情正在生成。</p> : null}
        </section>
      </div>
