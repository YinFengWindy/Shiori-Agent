...
export function ImageStudioPage({ activeRole, bridgeReady }: ImageStudioPageProps) {
  ...
  async function loadHistory(): Promise<void> {
    ...
    const records = Array.isArray(response.payload.records)
      ...
      : [];
    setHistory(records);
    setSelectedRecordId((current) => current || records[0]?.id || "");
  }

  useEffect(() => {
    let cancelled = false;
    void (async () => {
      const [settingsResult] = await Promise.all([window.miraDesktop.readSettings(), loadHistory()]);
      if (cancelled) return;
  ...
  return (
    <section className="image-studio-page scrollbar-soft h-full overflow-y-auto bg-[#F7F8FB]">
      <div className="mx-auto grid min-h-full w-full max-w-[1280px] grid-cols-[360px_minmax(0,1fr)_300px] gap-5 px-8 pb-10 pt-6">
        <ImageFormPanel
        ...
        <ImagePreviewPanel
          ...
          requestSummary={{
            mode: form.mode,
            width: form.sizePreset === "custom" ? (form.customWidth || "-") : (form.sizePreset === "landscape" ? "1216" : form.sizePreset === "portrait" ? "832" : "1024"),
            height: form.sizePreset === "custom" ? (form.customHeight || "-") : (form.sizePreset === "landscape" ? "832" : form.sizePreset === "portrait" ? "1216" : "1024"),
            steps: form.steps || "-",
            model: form.model || "-",
            seed: form.seed || "-",
          }}
