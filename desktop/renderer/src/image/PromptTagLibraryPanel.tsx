      ...
      <div className="grid min-h-[520px] grid-cols-[280px_minmax(0,1fr)] overflow-hidden rounded-lg border border-[#DDE4EC] bg-white shadow-[0_12px_32px_rgba(31,41,55,0.06)]">
        <aside className="border-r border-[#E5EAF0] bg-[#F8FAFC] p-3">
          <div className="grid gap-2">
            ...
            <div className="flex items-center gap-2"><select className="min-w-0 flex-1 rounded-md border border-[#D8DFE7] bg-white px-2 py-2 text-xs outline-none" value={categoryFilter} onChange={(event) => setCategoryFilter(event.target.value)}><option value="">全部分类</option>{categories.map((category) => <option key={category} value={category}>{category}</option>)}</select><button className="flex shrink-0 items-center gap-1.5 rounded-md bg-[#2F6FED] px-3 py-2 text-xs font-medium text-white transition hover:bg-[#245BC2]" type="button" onClick={() => { setDraft(emptyDraft); setSelectedId(""); setIsCreating(true); setError(""); }}><PlusIcon className="h-3.5 w-3.5 fill-current" />新建</button></div>
          </div>
