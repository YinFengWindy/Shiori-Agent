...
const storyPrimaryButtonClass = "inline-flex items-center justify-center rounded-md border border-[#A93E6A] bg-[#A93E6A] px-[18px] py-3 text-white transition-colors hover:bg-[#902B57] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#E5A9C0] disabled:cursor-default disabled:opacity-50";

/** Renders the four-step form that creates one Story database entry. */
      ...
      <main className="relative z-10 min-h-0 flex-1 overflow-y-auto px-5 py-8 sm:px-8">
        <div className="mx-auto grid max-w-2xl gap-7">
          ...
          {step === "role" ? <section className="grid gap-3">{roles.map((role) => {
            ...
            return <button key={role.id} className={cx("flex min-h-20 items-center gap-3 rounded-md border bg-white/70 p-4 text-left transition-colors", selected ? "border-[#B64B75] bg-[#FFF0F6] shadow-[0_5px_16px_rgba(161,54,100,0.12)]" : "border-[#E9C4D5] hover:border-[#CF7898] hover:bg-white")} type="button" aria-pressed={selected} onClick={() => setInput((current) => ({ ...current, roleId: role.id }))}>{role.avatarUrl ? <img className="h-11 w-11 rounded-md border border-[#E5B8C9] object-cover" src={role.avatarUrl} alt="" /> : <UserCircle className="h-11 w-11 text-[#A85E7D]" weight="thin" />}<span className="min-w-0 flex-1"><strong className="block font-serif text-base text-[#5E2841]">{role.name}</strong><span className="block truncate text-xs text-[#8B6676]">{role.description}</span></span>{selected ? <Check className="text-[#A93E6A]" weight="bold" /> : null}</button>;
          })}</section> : null}
          {step === "setting" ? <section className="grid gap-5 border-y border-[#E9C4D5] py-5"><label className="grid gap-1.5 text-xs text-[#8B6676]">剧情名称<input className={storyInputClass} value={input.title} onChange={(event) => setInput((current) => ({ ...current, title: event.target.value }))} /></label><label className="grid gap-1.5 text-xs text-[#8B6676]">开场背景<textarea className={cx(storyInputClass, "min-h-32 resize-y")} value={input.background} onChange={(event) => setInput((current) => ({ ...current, background: event.target.value }))} /></label><label className="grid gap-1.5 text-xs text-[#8B6676]">开始时间（北京时间）<input className={storyInputClass} type="datetime-local" value={input.startsAt} onChange={(event) => setInput((current) => ({ ...current, startsAt: event.target.value }))} /></label></section> : null}
          {step === "player" ? <section className="grid gap-5 border-y border-[#E9C4D5] py-5"><div className="grid gap-4 sm:grid-cols-2"><label className="grid gap-1.5 text-xs text-[#8B6676]">名称<input className={storyInputClass} value={input.playerProfile.displayName} onChange={(event) => updateProfile("displayName", event.target.value)} /></label><label className="grid gap-1.5 text-xs text-[#8B6676]">身份<input className={storyInputClass} value={input.playerProfile.identity} onChange={(event) => updateProfile("identity", event.target.value)} /></label></div><label className="grid gap-1.5 text-xs text-[#8B6676]">外貌<input className={storyInputClass} value={input.playerProfile.appearance} onChange={(event) => updateProfile("appearance", event.target.value)} /></label></section> : null}
          {step === "review" ? <section className="divide-y divide-[#E9C4D5] border-y border-[#E9C4D5]"><div className="grid gap-1 py-4"><span className="text-xs text-[#A48090]">角色</span><strong className="font-serif text-[#5E2841]">{selectedRole?.name || "未选择"}</strong></div><div className="grid gap-1 py-4"><span className="text-xs text-[#A48090]">剧情</span><strong className="font-serif text-[#5E2841]">{input.title}</strong><span className="text-sm text-[#765667]">{input.background}</span></div><div className="grid gap-1 py-4"><span className="text-xs text-[#A48090]">玩家</span><strong className="font-serif text-[#5E2841]">{input.playerProfile.displayName}，{input.playerProfile.identity}</strong><span className="text-sm text-[#765667]">{input.playerProfile.appearance} · {input.startsAt}</span></div></section> : null}
          {error ? <div className="border border-[#D58A9F] bg-[#FFF0F4] px-3 py-2 text-sm text-[#9A365D]" role="alert">{error}</div> : null}
        </div>
      </main>
      <footer className="relative z-10 border-t border-[#E9C4D5] bg-white/60 px-5 py-4 sm:px-8"><div className="mx-auto flex max-w-2xl justify-between gap-3">{stepIndex ? <button className={cx(ghostButtonClass, "border-[#E5B8C9] bg-white/70 text-[#6C3E52] hover:bg-white focus:outline-none focus-visible:ring-2 focus-visible:ring-[#E5A9C0]")} type="button" disabled={busy} onClick={previous}>上一步</button> : <span />}{step === "review" ? <button className={storyPrimaryButtonClass} type="button" disabled={busy || !stepComplete} onClick={() => onCreate(input)}><span className="inline-flex items-center gap-2"><Sparkle weight="fill" />开始剧情</span></button> : <button className={storyPrimaryButtonClass} type="button" disabled={!stepComplete} onClick={next}><span className="inline-flex items-center gap-2">下一步<ArrowRight /></span></button>}</div></footer>
    </section>
  );
}
