...
export function StoryLoadingScreen({ mode, busy = true, error = "", elapsedMs = 250, loaded = 0, total = 0, onRetry, onBack }: StoryLoadingScreenProps) {
  const activeStage = mode === "listing" ? 0 : 1;
  const presentation = resolveStoryLoadingPresentation({ elapsedMs, loaded, total });
  return (
    <section className="relative grid h-full min-h-0 place-items-center overflow-hidden text-[#F6F0E8]" data-testid="story-loading-screen" aria-busy={busy}>
      {/* Full-screen background */}
      <div className="absolute inset-0 bg-cover bg-center bg-no-repeat" style={{ backgroundImage: `url(${STORY_MENU_BACKGROUND_URL})` }} />
      <div className="absolute inset-0 bg-black/50 backdrop-blur-sm" />
      <div className="relative z-10 w-[min(520px,calc(100%-48px))]">
        <div className="flex items-center gap-2 text-xs uppercase tracking-[0.28em] text-[#D49A76]"><GlobeHemisphereEast weight="duotone" />Story</div>
        <h1 className="mt-5 font-serif text-3xl text-[#F7EBDD]">正在打开剧情</h1>
        <p className="mt-2 text-sm text-white/50">请稍候，剧情正在准备。</p>
        <div className="mt-12 grid gap-4" aria-label="剧情加载阶段">
