...
describe("StoryArchiveSurface", () => {
  it("renders direct Story beats without Day, OC, or workspace semantics", () => {
    const markup = renderToStaticMarkup(<StoryArchiveSurface background={resolvedBackground} story={createStoryDetails()} error="" onOpenSettings={() => undefined} onExit={() => undefined} />);
  ...
  it("keeps the committed beat order from the Story repository", () => {
    ...
    const markup = renderToStaticMarkup(<StoryArchiveSurface story={story} error="" onOpenSettings={() => undefined} onExit={() => undefined} />);
