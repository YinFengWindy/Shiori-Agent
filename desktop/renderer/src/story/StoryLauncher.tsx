...
export function StoryLauncher({ busy = false, error = "", onCreateStory, onOpenLoad, onOpenSettings, onExit }: StoryLauncherProps) {
  ...
  return (
    <section className="relative h-full min-h-0 overflow-hidden bg-[#1D1520]" data-testid="story-launcher">
      <motion.div className="absolute inset-0 bg-cover bg-center bg-no-repeat" data-testid="story-menu-backdrop" initial={{ opacity: 0, scale: reducedMotion ? 1 : 1.06 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: reducedMotion ? 0 : 1.4, ease: "easeOut" }} style={{ backgroundImage: `url(${STORY_MENU_BACKGROUND_URL})` }} />
