...
describe("roleTaskFormState", () => {
  it("creates defaults in the supplied local timezone", () => {
    assert.deepEqual(createScheduleTaskFormData("Asia/Shanghai"), {
      name: "",
      tier: "instant",
      trigger: "at",
      when: "",
  ...
  it("maps a scheduled task into editable values", () => {
    const task: RoleTask = {
      ...
      cancellable: true,
      editable: true,
      schedule: { tier: "soft", trigger: "every", when: "0 9 * * *", content: "查询天气", timezone: "Asia/Shanghai" },
    };
  ...
  it("validates required fields but leaves schedule syntax to the backend", () => {
    const missing = validateScheduleTaskForm({ ...createScheduleTaskFormData("UTC"), timezone: "" });
    assert.deepEqual(Object.keys(missing).sort(), ["content", "name", "timezone", "when"]);
