  ...
  return (
    <div className="grid h-full min-h-0 grid-rows-[auto_minmax(0,1fr)_auto] rounded-[20px] bg-[#F1F5F9] p-4 text-sm text-[#334155]">
      <div className="flex items-center gap-2 pb-3">
        <button className={cx("grid h-8 w-8 place-items-center rounded-md transition hover:bg-white", focusResetClass)} type="button" aria-label="返回任务列表" onClick={onBack}>
      ...
      <div className="grid gap-2 pt-3">
        ...
        {confirmingCancel ? (
          <div className="flex items-center justify-end gap-2">
            ...
            <button className={cx("rounded-md px-3 py-2 text-xs transition hover:bg-white", focusResetClass)} type="button" disabled={cancelling} onClick={onDismissCancel}>返回</button>
          ...
          <div className="flex justify-end gap-2">
            {task.kind === "schedule" ? (
              <button className={cx("inline-flex items-center gap-1 rounded-md border border-[#D8DFE7] px-3 py-2 text-xs transition hover:border-[#B8C2CE] disabled:opacity-50", focusResetClass)} type="button" disabled={!task.editable} onClick={onEdit}>
                <PencilSimple className="h-4 w-4" />编辑
              </button>
            ) : null}
            {task.cancellable ? (
              <button className={cx("inline-flex items-center gap-1 rounded-md border border-[#E6B8B3] px-3 py-2 text-xs text-[#B42318] transition hover:border-[#B42318]", focusResetClass)} type="button" onClick={onBeginCancel}>
