...
export function RoleChannelBindingsPanel({ activeRoleId, bindings, onUpdate }: RoleChannelBindingsPanelProps) {
  ...
  return (
    <section className="grid gap-4 rounded-md border border-[#E5E7EB] bg-white p-5 text-sm text-[#1F2937] shadow-[0_12px_30px_rgba(15,23,42,0.06)]" data-testid="role-channel-config">
      <div className="flex items-center justify-between gap-3">
        <div><h2 className="font-medium">渠道绑定</h2><p className="mt-1 text-xs text-[#6B7280]">按顺序维护角色可使用的会话与群组。</p></div>
        <button className="rounded-md border border-[#D8DCE2] bg-white px-3 py-1.5 text-xs transition hover:border-[#2176FF] hover:text-[#2176FF]" type="button" onClick={() => updateBindings((current) => [...current, createRoleChannelBinding(activeRoleId)])}>添加</button>
      </div>
      {bindings.length ? bindings.map((binding, index) => (
        <div className="grid gap-2 rounded-md border border-[#E5E7EB] bg-[#F8FAFC] p-3" key={`${binding.channel}:${binding.chat_id}:${index}`}>
          <div className="grid gap-2 md:grid-cols-[120px_minmax(0,1fr)_auto_auto]">
            <select className={cx(inputClass, "border-[#D8DCE2] bg-white text-[#1F2937]")} value={binding.channel} onChange={(event) => updateBindings((current) => current.map((item, itemIndex) => itemIndex === index ? changeRoleBindingChannel(item, event.target.value, activeRoleId) : item))}>
            ...
            <input className={cx(inputClass, "border-[#D8DCE2] bg-white text-[#1F2937]")} value={binding.chat_id} placeholder="会话 / 群组 ID" readOnly={isDesktopRoleBinding(binding)} onChange={(event) => updateBindings((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, chat_id: event.target.value } : item))} />
            <div className="flex items-center justify-end gap-1">
              <button className="grid h-8 w-8 place-items-center rounded-md border border-[#D8DCE2] text-[#64748B] transition hover:border-[#2176FF] hover:text-[#2176FF] disabled:cursor-default disabled:opacity-35" type="button" onClick={() => updateBindings((current) => moveRoleChannelBinding(current, index, "up"))} disabled={index === 0} aria-label={`上移${roleBindingChannelLabel(binding.channel)}绑定`} title="上移"><ArrowUpIcon className="h-4 w-4 stroke-current" /></button>
          ...
          {!isDesktopRoleBinding(binding) ? <label className="grid gap-1.5 text-xs"><span>{roleBindingAllowFromLabel(binding.channel)}</span><input className={cx(inputClass, "border-[#D8DCE2] bg-white text-[#1F2937]")} value={binding.allow_from.join(", ")} onChange={(event) => updateBindings((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, allow_from: event.target.value.split(",").map((value) => value.trim()).filter(Boolean) } : item))} /></label> : null}
        </div>
      )) : <div className="rounded-md border border-dashed border-[#D8DCE2] px-4 py-6 text-center text-xs text-[#6B7280]">尚未绑定渠道</div>}
    </section>
  );
}
