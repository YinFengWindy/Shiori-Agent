...
一个 Story 段结束时产生的不可变记录，也是向普通模式提交角色故事回忆的边界。继续会在原 Story 中开始新段，分支才会创建新的 Story。
...
一个 Story 段在角色经历时间轴上的发生位置，与 Story 的创建时间无关。它决定角色在该段开始时可以知道的记忆和所处的关系阶段。
_避免使用_：把当前关系状态直接当作历史 Story 的开场状态

**Story 后续连续性（Story Future Continuity）**：
时间锚点之后已经明确发生的角色经历和关系结果。角色在过去的 Story 中不能知道这些内容，但 Story 的发展不能使这些后续历史变得不可能。
