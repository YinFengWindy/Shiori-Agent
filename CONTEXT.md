...
Shiori 是基于 Akashic 的角色型 Agent 助手。项目的核心不是单一聊天界面，而是围绕角色、关系、会话、记忆、主动行为、多渠道和桌面端组成的持续运行系统。

## 领域语言

**已提交 Story 节拍（Committed Story Beat）**：
Story 中已经正式发生的最小叙事单位，也是 Story 自身的正式记录。只有完成提交的 Story 节拍属于故事记录；模型生成草稿和待玩家确认的结果都不属于已发生剧情。
...
一个 Story 段结束时产生的不可变记录。它冻结该段的正式剧情和正式导入角色的故事回忆；继续会在原 Story 中开始新段，分支才会创建新的 Story。
_避免使用_：把归档称为整个 Story 的终止

**Story 分支（Story Fork）**：
从某个 Story 归档创建的新 Story 路线。它复制归档及之前的已提交剧情、快照和不可变资源引用，并生成新 ID 映射；归档之后的连续性、草稿、待决策、播放状态和未完成资源任务都不属于分支。
_避免使用_：把分支当作继续；复制演出 cursor、未完成 TTS/CG 或旧 Akasha 投递状态

**角色故事回忆（Story Archive Memory）**：
Story 归档时按正式导入角色视角生成并冻结的自然语言回忆。它属于 Story 归档，即使尚未投递到 Akasha 也不会重新生成或改变内容。
...
_避免使用_：把待决策选项或玩家点击直接写成已发生事实；把待决策模型混入 Story 节拍
...
**正常模式当前时间候选（Normal-mode Current-time Candidate）**：
选定正式角色的正常模式记忆中，能够明确表达“当前/截至某时刻上下文”的精确发生时间，并且保留原始来源引用的候选。它可以用于解析 Story 的“现在”；历史事件时间、只有模型摘要的时间、记忆记录时间和系统时钟都不能自动代替它。同一上下文范围的候选必须一致，冲突时需要玩家确认。
_避免使用_：把最新写入的记忆时间、会话更新时间或模型推断的日期直接当作 Story 时间锚点
