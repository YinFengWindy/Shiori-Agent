...
export function AkashaMemoryDashboard({ roleId, bridgeReady, client, host }: PluginRoleMemoryComponentProps) {
  ...
  const readList = useCallback((id: string, options: RoleSemanticQuery) => client.call<RoleSemanticList>("roles.memory.semantic.list", { role_id: id, q: options.q, sort_by: options.sort_by, sort_order: options.sort_order, page: options.page, page_size: options.page_size }), [client]);
  ...
  return <section className="grid gap-5" aria-label="角色记忆" data-testid="role-memory-panel">
    <header className="flex items-center justify-between gap-3">
      <h2 className="m-0 text-title-sm text-ink">Akasha 记忆</h2>
      <MemoryRefreshButton disabled={!roleId || !bridgeReady} onRefresh={refresh} />
    </header>
    <RoleSemanticMemoryPane query={query} onQuery={(next) => { setQuery(next); setSelectedId(""); }} supportsStructuredFilters={false} list={semantic.list} loading={semantic.listLoading} error={semantic.listError} selectedId={selectedId} onSelect={setSelectedId} detail={semantic.detail} detailLoading={semantic.detailLoading} detailError={semantic.detailError} renderError={(message) => <host.ui.InlineError message={message} />} />
