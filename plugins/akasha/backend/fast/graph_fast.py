...
from ..core import (
    ...
    has_user_turn as _orig_has_user_turn,
    recover_resource,
)
...
def _fast_graph_expand_candidates(
    query_vec,
    nodes,
    direct_scores,
    fan,
    ...
    def _eff(src_key, dst_key, weight):
        ...
        return effective_edge_weight(
            weight, edges_meta.get((src_key, dst_key), 0.0), now_ts
        )
    ...
    for seed_key in graph_seed_keys:
        ...
        out_strength = sum(
            _eff(seed_key, dst_key, w) for dst_key, w in raw_neighbors.items()
        )
        if out_strength <= 0:
            continue
        scored_neighbors = []
        for key, edge_weight in raw_neighbors.items():
            if (
                key not in nodes
                or key in seed_set
                or not cached_has_user_turn(source_cursor, key)
            ):
                continue
...
def install(store):
    ...
    print(
        "[graph_fast] installed (in_strength 增量 + has_user_turn 缓存 + edges_by_src/fan 视图)",
        flush=True,
    )
