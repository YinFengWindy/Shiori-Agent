...
将本插件目录复制到 Shiori 仓库外。准备私有 wheelhouse，其中必须有 `shiori-agent`、`shiori-plugin-testkit`、`shiori-plugin-default-memory`、`shiori-plugin-desktop-pet` 的 0.1.0 wheel。桌宠包仅作为独立启停的测试依赖。这些私有包不发布到 PyPI；其余第三方依赖由包元数据解析。
