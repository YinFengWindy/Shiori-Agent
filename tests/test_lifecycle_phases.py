...
@pytest.mark.asyncio
async def test_before_turn_group_session_inherits_role_runtime_config():
    bus = EventBus()
    role_session = _DummySession("role:mira")
    role_session.metadata["role_runtime_config"] = {"nsfw_memory_enabled": True}
    group_session = _DummySession("role:mira:group:100:member:u1")

    sessions = {
        role_session.key: role_session,
        group_session.key: group_session,
    }
    session_mgr = SimpleNamespace(
        get_or_create=lambda key: sessions[key],
    ...
    group_session.messages = [{"role": "user", "content": f"u{i}"} for i in range(30)]

    ctx = await phase.run(state)

    assert ctx.abort is False
    assert group_session.metadata["role_runtime_config"] == {
