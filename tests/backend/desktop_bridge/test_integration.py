...
# coverage that used to live here now targets the plugin's own RPC handlers
# directly — see plugins/novelai/tests/test_rpc.py. DesktopBridgeService no
# longer wires an image service at all: manual generation, history, prompt
# tags and message-media regeneration are entirely owned by the novelai
# plugin's ``plugin.novelai.*`` RPC methods.
