...
视觉事实使用自然语言描述，保留角色可见外观与当前动作；不要输出供应商标签或生图参数。closed 和 none 的 scene_key、visual_key、visual_description 必须为空字符串。"""
...
def _build_decision_prompt(decision_input: SceneDecisionInput) -> str:
    ...
    return (
        ...
        "同一场景内人物动作、姿势、位置关系、服装、道具或光线有实质变化时，返回 same 并提供新 visual_key；"
        "完全相同的画面沿用 current_visual_key。告别、睡觉、离场或场景明确结束时为 closed；"
        "没有 current_scene_key 且本轮没有可见场景时为 none。"
        "visual_description 是当前定格的中立自然语言描述，不是绘图指令。\n\n"
