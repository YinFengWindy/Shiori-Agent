        ...
        *,
        ...
        with diagnostic_context(session=key, flow="passive", turn=turn_id):
            ...
            try:
                ...
                # 用户消息的落库发生在 before_turn 链最前面（issue #306，见
                # before_turn._PersistPendingUserMessageModule，紧跟在
                # acquire_session 之后）：格式非法、provider 报错、超出输入预算
                # （不论是 reasoner 内部检查还是 before_turn 的
                # _MemoryContextGuardModule）、before_reasoning 窗口内的取消等
                # 回复阶段失败都发生在那一步之后，不会再导致用户这一轮消息从
                # 历史里消失。助手侧仍然只在成功提交时落库，见 AfterReasoning 的
                # persist_asst / append_messages 与 formal_role_reply 的
                # mood-gated 原子提交。
                ...
                # 用户消息早在 before_turn 链最前面就已经落库（见上面的注释），
                # 这里不需要再做任何事——单纯让异常继续传播。
                raise
