    ...
    _schema(
        ...
        ),
        {
            ...
            "properties": {
                "query": {
                    "type": "string",
                    "description": "假设性陈述，描述用户对这条内容的正面或负面评价",
                },
            },
            "required": ["query"],
        },
...
def _get_content(ctx: AgentTickContext, args: dict) -> str:
    ...
    if unknown:
        logger.warning(
            "[proactive_v2] get_content: unknown item_ids=%s (valid=%s)",
            unknown,
            sorted(valid),
        )
        return json.dumps(
...
async def _web_fetch(
    ctx: AgentTickContext, args: dict, *, web_fetch_tool, max_chars: int
) -> str:
    if web_fetch_tool is None:
        return json.dumps(
            {"error": "web_fetch tool not configured"}, ensure_ascii=False
        )
...
async def _web_search(ctx: AgentTickContext, args: dict, *, web_search_tool) -> str:
    if web_search_tool is None:
        return json.dumps(
            {"error": "web_search tool not configured"}, ensure_ascii=False
        )
...
def _mark_not_interesting(ctx: AgentTickContext, args: dict) -> str:
    ...
    if unknown and valid:
        logger.warning(
            "[proactive_v2] mark_not_interesting: unknown item_ids=%s", unknown
        )
        return json.dumps(
            {
                "error": f"以下 id 不在本轮候选列表中，无法标记：{unknown}。本轮有效 id：{sorted(valid)}"
...
def _finish_turn(ctx: AgentTickContext, args: dict) -> str:
    ...
    if decision == "reply":
        if not ctx.draft_message.strip():
            raise ValueError(
                "finish_turn(decision=reply) requires prior message_push call"
            )
...
async def dispatch(
    ...
    if tool_name == "web_fetch":
        return await _web_fetch(
            ctx, args, web_fetch_tool=deps.web_fetch_tool, max_chars=deps.max_chars
        )
