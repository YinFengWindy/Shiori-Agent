...
import { storySamples } from "./demoContent";
...
function isSavedStory(value: unknown): value is SavedStory {
  ...
  return "turns" in value && Array.isArray(value.turns) && value.turns.length > 0 && value.turns.length <= storySamples.length
...
export function restoreDemoStories(storage: DemoStorage) {
  ...
  return saved.map((entry) => {
    let story = createDemoStory({ creation_id: entry.id.slice(5), title: entry.title, background: entry.background, story_date: entry.currentStoryDate, time_band: entry.currentTimeBand, player_profile: entry.playerProfile });
...
export function persistDemoStories(storage: DemoStorage, stories: Iterable<StoryDetails>) {
  const saved: SavedStory[] = [...stories].map(({ id, title, background, playerProfile, currentStoryDate, currentTimeBand, turns }) => ({
    id, title, background, playerProfile, currentStoryDate, currentTimeBand, turns: turns.map(({ kind, input }) => ({ kind, input })),
  }));
