...
export function ShowcaseChat({ chat, onStory }: { chat: ReturnType<typeof createDemoChat>; onStory: () => void }) {
  ...
  return <div className="showcase-chat flex h-full min-h-0">
    ...
    <main className="showcase-chat-main relative grid min-h-0 min-w-0 flex-1 grid-rows-chat overflow-hidden rounded-md border border-line-soft bg-surface-soft">
      <ChatHeader activeRole={demoRole} detailRole={null} title={demoRole.name} onOpenRoleDetail={noAction} />
      <button type="button" className={`${iconButtonClass} showcase-secondary absolute right-3 top-2 z-[3]`} aria-label={statusOpen ? "收起角色状态" : "展开角色状态"} aria-expanded={statusOpen} onClick={() => setStatusOpen((open) => !open)}><SidebarSimple className="h-5 w-5" /></button>
