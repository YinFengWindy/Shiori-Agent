...
type StoryCreateStepProps = {
  ...
  onChangeSetting: (field: "title" | "background" | "storyDate" | "timeBand", value: string) => void;
...
export function StoryCreateStep({ step, roles, input, selectedRole, reducedMotion, onSelectRole, onChangeSetting, onChangeProfile }: StoryCreateStepProps) {
  ...
  if (step === "setting") {
    return (
      <section className="grid gap-5 border-y border-[#E9C4D5] py-5" data-testid="story-create-step">
        ...
        <label className="grid gap-1.5 text-xs text-[#8B6676]">开始日期<input className={storyInputClass} type="date" value={input.storyDate} onChange={(event) => onChangeSetting("storyDate", event.target.value)} /></label>
  ...
  return (
    <section className="divide-y divide-[#E9C4D5] border-y border-[#E9C4D5]" data-testid="story-create-step" aria-label="剧情总览">
      ...
      <div className="grid gap-1 py-4"><span className="text-xs text-[#A48090]">剧情</span><strong className="font-serif text-[#5E2841]">{input.title}</strong><span className="whitespace-pre-wrap text-sm text-[#765667]">{input.background}</span><span className="text-xs text-[#8B6676]">开始日期：{input.storyDate}</span><span className="text-xs text-[#8B6676]">开始时段：{normalizeStoryTimeBand(input.timeBand)}</span></div>
