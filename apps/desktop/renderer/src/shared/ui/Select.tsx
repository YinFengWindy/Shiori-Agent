...
export interface SelectOption {
  ...
  triggerLabel?: string;
  /** Leading visual (e.g. an avatar) shown in the list row and, once selected, in the trigger. */
  icon?: ReactNode;
  disabled?: boolean;
}
...
export function Select({ value, options, onValueChange, disabled, id, name, className, ...aria }: SelectProps) {
  // Only icon-bearing option lists replace Base UI's default label rendering in the trigger.
  const renderValue = options.some((option) => option.icon)
    ? (current: string) => {
      ...
      return (
        <span className="flex min-w-0 items-center gap-2">
          ...
          <span className="min-w-0 truncate">{option.triggerLabel ?? option.label}</span>
        </span>
      );
    }
    : undefined;
  return (
    ...
    >
      ...
      >
        <SelectPrimitive.Value className="min-h-[1lh] min-w-0 truncate">{renderValue}</SelectPrimitive.Value>
