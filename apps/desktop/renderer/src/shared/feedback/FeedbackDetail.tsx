...
export function FeedbackDetail({ detail, open, onToggle }: { detail: string; open: boolean; onToggle: () => void }) {
  return (
    <>
      ...
      {open ? (
        <pre className="scrollbar-soft m-0 max-h-32 overflow-auto whitespace-pre-wrap break-words rounded-md bg-white/60 p-2 text-left font-mono text-caption text-ink-secondary">{detail}</pre>
      ) : null}
    </>
  );
}
