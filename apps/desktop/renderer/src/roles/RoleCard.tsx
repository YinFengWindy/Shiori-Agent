 ...
 * corner overflow menu, so no button ever nests inside another. Hovering with
 * a mouse tilts it (TiltedCard); its portrait, name and intro carry
...
export function RoleCard({ role, active, bridgeReady, pendingCardAction, onOpen, onGoToChat, onDelete }: RoleCardProps) {
  ...
  return (
    <TiltedCard className="rounded-lg">
      <article
        className={cx(
          "group relative isolate aspect-[4/5] overflow-hidden rounded-lg border bg-surface shadow-soft transition-[box-shadow,border-color] duration-base ease-out-soft hover:shadow-pop focus-within:shadow-pop",
