        ...
        <div className="grid gap-3 sm:grid-cols-[168px_minmax(0,1fr)]">
          <div className={cx(roleFieldLabelClass, "min-w-0")}>
            ...
            {readOnly
              ...
              : <Select aria-label="渠道" className={roleFieldClass} value={binding.channel} onValueChange={(value) => updateThis((item) => changeRoleBindingChannel(item, value, activeRoleId))} options={roleBindingChannelOptions(channels, binding.channel)} />}
          </div>
          <label className={cx(roleFieldLabelClass, "min-w-0")}><span className="flex min-h-5 items-center">{chatIdCopy.label}</span><input className={readOnlyFieldClass} value={binding.chat_id} placeholder={chatIdCopy.placeholder} readOnly={desktopBinding || readOnly} onChange={(event) => updateThis((item) => ({ ...item, chat_id: event.target.value }))} /></label>
        </div>
        {!desktopBinding
...
export function RoleChannelBindingsPanel({ activeRoleId, bindings, channels, onUpdate, onOpenPluginSettings }: RoleChannelBindingsPanelProps) {
  ...
  return (
    <RoleEditorSection
      ...
      action={<button className={rolePanelGhostButtonClass} type="button" disabled={channels === null} onClick={() => updateBindings((current) => [...current, createRoleChannelBinding(activeRoleId, defaultRoleBindingChannel(channels))])} aria-label="添加渠道绑定"><Plus className="h-4 w-4" weight="bold" />添加</button>}
    >
      {bindings.length
