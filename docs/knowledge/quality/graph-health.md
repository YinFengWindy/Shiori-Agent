...
- 基于提交：`937adfdc`。
- 语料：546 个文件，约 235,835 词。
- 最终可查询图：6,235 个节点、17,029 条边、252 个社区。
- `graph.html` 使用社区聚合视图：252 个社区节点、1,010 条跨社区边。

## 首次全量构建基线

- 语料：535 个文件，约 234,012 词；其中代码 513 个、文档 22 个。
- AST：6,058 个节点、19,269 条原始边。
- 文档语义：90 个节点、87 条边、2 个超边。
- 最终图：6,148 个节点、16,957 条边、240 个社区。
...
这些诊断值来自首次全量构建的 `health.json`。提交后增量更新不会重写该诊断文件，因此不能把它们误称为当前增量图的精确健康值。图谱仍适合导航和缩小搜索范围，但不能用边数量证明完整性。对删除、迁移、权限、自动触发等高风险结论，必须回到源码和测试核验。
